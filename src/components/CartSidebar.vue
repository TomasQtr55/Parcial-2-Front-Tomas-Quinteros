...
import { storeCartBooks } from '@/stores/cartStore';

const store = storeCartBooks()

const listCarito = computed(()=> store.listacarrito)

const cantidad = computed(()=> store.cantidadCarrito)
const eliminar =(idlibro: number) =>{
    store.eliminarDelCarrito(idlibro)
}
...
<template>
    ...
    <aside
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4">
            <div v-for="libro in listCarito" :key="libro.id"
                class="flex items-center justify-between bg-emerald-50 border border-emerald-100 p-3 rounded-lg">
                <div>
                    <h3 class="font-medium text-gray-800">Libro {{ libro.title}}</h3>
                    <p class="text-sm text-gray-500">$ {{ libro.price}}</p>
                </div>
                <button @click="eliminar(libro.id)" class="text-red-500 hover:text-red-700">
